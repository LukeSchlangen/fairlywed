app.factory("PhotographerSearchFactory", function (PackagesFactory, $http, $stateParams, $state) {
    ...
    function updateSearchParameters() {
        search.parameters.location = search.parameters.location || $stateParams.location || "Minneapolis, MN, USA";
        ...
        search.parameters.date = search.parameters.date ? new Date(search.parameters.date) : ($stateParams.date ? new Date($stateParams.date) : saturdayOneYearFromNow());
    ...
    var currentSubvendor = { details: { name: 'Minneapolis Wedding Photography' } };
