...
router.get('/', function (req, res) {
    ...
    if (req.query.photos && req.query.photos.length > 0) {
        ...
        saveLikes(req, res, photos, firebaseUserId, getImagesWithUserId);
                    ...
                    function (err, hasDoneMatchmakerBefore) {
                        ...
                        if (err) {
                            ...
                            if (hasDoneMatchmakerBefore.rows[0].exists) {
                                // get Latest info on Pref for user and get images;
                                getImagesWithUserId(req, res, firebaseUserId)
                            } else {
                                // get random images
                                getRandomImages(res);
                            }
...
function saveLikes(req, res, photos, firebaseUserId, cb) {
...
function getImagesWithUserId(req, res, firebaseUserId) {
    var orderBy = simpleRanker.orderBy(firebaseUserId);
    pool.connect(function (err, client, done) {
        if (err) {
            console.log('Error connecting to database', err);
            res.sendStatus(500);
            ...
            client.query(`SELECT * FROM subvendor_images 
                ...
                function (err, images) {
                    ...
                    if (err) {
                        ...
                        simpleRanker.reccommendedPhotographers(req, res, firebaseUserId, images.rows, vendorSearch);
                    }
                });
        }
    });
